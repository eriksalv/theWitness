...
public class GameCollection {
	...
	// tar inn et navn på collection og varargs for games i collection
	public GameCollection(String name, Game... games) {
		...
		// legger til denne GameCollection i hvert game sin GameCollection felt
		this.games.values().forEach(game -> game.addGameCollection(this));

		this.name = name;
	}

	// lager en ny gamecollection som består av de standard, uendra nivåene i config
	// mappa.
	public static final GameCollection newGame() throws FileNotFoundException, URISyntaxException {
	...
	public int getGameIndex() {
		return isGamesWon.keySet().stream()
				...
				.findFirst().orElse(1); // finner det første nivået som ikke er vunnet enda, og hvis ikke det fins,
										// returner det første nivået
	}
	...
	public void addGame(Game game, int level) {
		if (level >= levelCount + 1) { // skal ikke være mulig å legge til et game med 2 eller mer level høyere enn det
										// siste i games lista
	...
	public void addGame(Game game, boolean isGameWon) {
		games.put(levelCount, game); // legger til et nytt game eller overskriver et som allerede eksisterer (med
										// samme LEVEL)
	...
	public void removeGame(int level) {
		...
		// erstatter alle key-entry par der verdien til key er større eller lik verdien
		// til level med neste key-verdi i map.
		games.keySet().stream().filter(key -> key.intValue() > level)
				.forEach(key -> games.replace(key - 1, games.get(key)));
		isGamesWon.keySet().stream().filter(key -> key.intValue() > level)
				.forEach(key -> isGamesWon.replace(key - 1, isGamesWon.get(key)));
		// fjerner siste key-entry par.
		games.remove(games.size());
		isGamesWon.remove(isGamesWon.size());

		// minker nivået med 1 til alle games etter gamet som ble fjernet
		games.values().stream().filter(g -> g.getLevel() > level).forEach(g -> g.setLevel(g.getLevel() - 1));
		levelCount--;
	}
	...
	public boolean hasNextLevel(int currentLevel) {
		return getIsGamesWon().get(currentLevel) && getIsGamesWon().containsKey(currentLevel + 1);// skjekker at
																									// isGameWon er satt
																									// til true før
																									// neste game
	}
