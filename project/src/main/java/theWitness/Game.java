...
public class Game extends Grid {
	...
	public void clear() {
		getGrid()
				.forEach(list -> list.stream()
						.filter(tile -> !Character.toString(tile.getType()).matches("[S@><_PC]")) // tile typer som ikke
																									// skal resettes
	...
	private void drawLine(int dx, int dy, String move) {
		...
		if (firstMove) {
			...
			getStreamFromIterator().filter(tile -> tile.isStart()).forEach(tile -> {
				tile.setMovedLine();
				moves.put(tile, move); // legger til en moveOrder for start-tile. Det er ikke så farlig at denne ikke
										// er riktig, bare at den er der.
			});
			firstMove = false;
		}
		...
		// Lager en arraylist av settet av keys i moves for å hente ut den siste Tile
		// som ble lagt til
		...
		if (movesList.size() >= 2 && targetTile == movesList.get(movesList.size() - 2)) { // Hvis man går tilbake
			...
			if (targetTile.isGoal()) {
				if (checkCorrectPath()) {
					...
					if (gameCollection != null) {
						gameCollection.gameStateChanged(this, isGameWon); // informerer gameCollection om at isGameWon
																			// er true
					}
				} else {
					isGameOver = true;
			...
			if (movesList.size() >= 3) {
				movesList.get(movesList.size() - 3).setMovedLine(); // ruten to steg bak den siste skal bli "vanlig"
																	// igjen slik at den får kollisjon
			}
		}
		if (isGameWon) {
	...
	public static void main(String[] args) {
		 ...
		 * System.out.println(game.getMovedLine().get(i).getX() + "," +
		 * game.getMovedLine().get(i).getY());
		 * }
